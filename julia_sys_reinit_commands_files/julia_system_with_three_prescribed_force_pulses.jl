...
function forcing_model_3!(σ,T,t,fr::AreaRegionCache,phys_params)
    σt = phys_params["sigma_3"]
    t0 = phys_params["t0_3"]
    modfcn = Gaussian(σt,sqrt(π*σt^2)) >> t0
    σ .= modfcn(t)*fr.generated_field()
end
...
my_params["t0_2"] = 1.0
my_params["t0_3"] = 1.5
my_params["CFL"] = 0.5 # Because the default timestep_func does not account for point forcing in the flow
